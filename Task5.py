...
class contact:
    ...
    def search(self):
        ...
        if search_query:
            results = [contact for contact in self.contacts if
                       search_query.lower() in contact['Name'].lower() or search_query in contact['Mobile']]
            if results:
    ...
    def update(self):
        name_to_update = simpledialog.askstring("Update", "Enter the name of the contact to update: ")
        if name_to_update:
            for contact in self.contacts:
                if contact['Name'] == name_to_update:
                    new_name = simpledialog.askstring("Update", "Enter the new name: ")
                    new_phone = simpledialog.askstring("Update", "Enter the new phone number: ")
                    new_email = simpledialog.askstring("Update", "Enter the new email ID: ")
                    new_address = simpledialog.askstring("Update", "Enter the new address: ")
                    contact['Name'] = new_name
                    contact['Mobile'] = new_phone
                    contact['Email ID'] = new_email
                    contact['Address/Place'] = new_address
                    messagebox.showinfo("Success", "Contact updated successfully!")
                    return
            messagebox.showinfo("Update", "Contact not found.")

    def delete(self):
        name_to_delete = simpledialog.askstring("Delete", "Enter the name of the contact to delete: ")
        if name_to_delete:
            for contact in self.contacts:
